...
func CreateConstrant(c *gin.Context, db *sql.DB) {
	...
	_, err := execDB(db, query, constraint.Worker1, constraint.Worker2, constraint.Note)
	...
	if err != nil {
		...
		case hasPostgresCode(err, pgNotNullViolation):
	...
	if worker1Id != "" && worker2Id != "" {
		...
		if worker1FirstName != "" && worker2FirstName != "" {
			...
			if worker1FirstName != "" {
				...
				rows, err = queryDB(db, query, worker2FirstName, worker2LastName, worker2FirstName, worker2LastName)
			}
		} else {
...
func EditConstraints(c *gin.Context, db *sql.DB) {
	...
	result, err := execDB(db, query, constraint.Worker1, constraint.Worker2, constraint.Note, id)
